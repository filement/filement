...
*/
int aes_encrypt(const unsigned char *pt, unsigned char *ct, struct aes_context *skey)
{
...
/**
  ...
  @return CRYPT_OK if successful
*/
int aes_decrypt(const unsigned char *ct, unsigned char *pt, struct aes_context *skey)
{
    uint32_t s0, s1, s2, s3, t0, t1, t2, t3, *rk;
    int Nr, r;
...
/*int main(void)
{
 ...
 struct aes_context key;
 unsigned char tmp[2][16];
 int i, y;
 
 for (i = 0; i < (int)(sizeof(tests)/sizeof(tests[0])); i++) {
	aes_setup(tests[i].key, tests[i].keylen, 0, &key);
  
    aes_encrypt(tests[i].pt, tmp[0], &key);
    aes_decrypt(tmp[0], tmp[1], &key);
    if (memcmp(tmp[0], tests[i].ct, 16) || memcmp(tmp[1], tests[i].pt, 16)) { 
      ...
      for (y = 0; y < 1000; y++) aes_encrypt(tmp[0], tmp[0], &key);
      for (y = 0; y < 1000; y++) aes_decrypt(tmp[0], tmp[0], &key);
