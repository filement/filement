...
// Device UUID requires special parsing.
# include "protocol.h"
# include "cache.h"
#endif
...
#if defined(DEVICE) || defined(PUBCLOUD)
// Cover http_parse_uri_path to handle special URI cases.

// Each tiny URL starts with /~ and ends in / with everything after this slash ignored.
#define TINYURL_PREFIX '~'
#define TINYURL_SUFFIX '/'

#define http_parse_uri_path(request, start) http_parse_uri_path_device((request), (start))

static int http_parse_uri_path_device(struct http_request *restrict request, size_t start)
{
# if !defined(OS_WINDOWS)
...
# endif
	...
	// Make sure there is UUID in the request and it matches the UUID of the device.
	if ((request->URI.length - start) < (UUID_LENGTH + 1)) return NotFound;
	const char *uuid = request->URI.data + start + 1;
	if (memcmp(uuid, UUID.data, UUID_LENGTH) || (uuid[UUID_LENGTH] != '/')) return NotFound;
	start += 1 + UUID_LENGTH;

	// Detect tiny URL and fetch its corresponding real URL.
	if ((request->URI.length >= (start + 2)) && (request->URI.data[start + 1] == TINYURL_PREFIX))
	{
		// Make sure the tiny URL is in the expected format.
		const char *tiny = request->URI.data + start + 2;
		if (((request->URI.length - start - 2) < (CACHE_KEY_SIZE + 1)) || (tiny[CACHE_KEY_SIZE] != TINYURL_SUFFIX))
			return NotFound;

		const struct cache *cache = cache_use(tiny);
		if (!cache) return NotFound;
		union json *url = cache->value;
		if ((json_type(url) != STRING) || (url->string_node.data[0] != '/')) // TODO is the first check necessary
		{
			cache_finish(cache);
			return NotFound;
		}
		...
		request->URI.length = 1 + UUID_LENGTH + url->string_node.length;
		request->URI.data = malloc(request->URI.length + 1);
		if (!request->URI.data)
		{
			cache_finish(cache);
			return ServiceUnavailable;
		...
		request->URI.data[0] = '/';
		*format_bytes(format_bytes(request->URI.data + 1, UUID.data, UUID_LENGTH), url->string_node.data, url->string_node.length) = 0;
		cache_finish(cache);
		return (http_parse_uri_path)(request, 1 + UUID_LENGTH);
	}

	return (http_parse_uri_path)(request, start);
}
...
int http_parse_uri(struct http_request *restrict request)
{
	...
	if (!length) // Empty URI
	{
		...
		request->path.data = memdup("/", request->path.length);
...
int http_parse_old(struct http_request *restrict request, struct stream *restrict stream)
{
